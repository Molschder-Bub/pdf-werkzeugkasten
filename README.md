...
Der Werkzeugkasten trägt die **sellgrad-Marke**: Favicon, Marke in der Sidebar
und Lockup im Footer sind fest in `index.html` eingebettet. Sie gehören zum
Tool und dürfen beim Weitergeben, Forken oder Selbst-Hosten **nicht entfernt,
ausgeblendet oder ersetzt** werden.

Das wird automatisch überwacht: Die GitHub Action **„Branding-Schutz"**
(`.github/workflows/branding.yml`) läuft bei jedem Push und jedem Pull Request
und schlägt fehl, sobald Favicon, Sidebar-Marke, Footer-Lockup oder die
Copyright-Zeile fehlen oder das Logo per CSS unsichtbar gemacht wurde.

Selbst prüfen:

```bash
python3 .github/check-branding.py
```
